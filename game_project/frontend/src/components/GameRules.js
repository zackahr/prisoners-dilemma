...
function GameRules({ setMatchId, setPlayerFingerprint }) {
  ...
  useEffect(() => {
    // Generate a fingerprint based on browser info
    const generateFingerprint = () => {
      ...
      const fingerprintData = `${userAgent}-${language}-${screenWidth}-${screenHeight}`
      return btoa(fingerprintData).substring(0, 12)
    }
  ...
  const startGame = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/create_match/", {
        ...
        body: JSON.stringify({
          game_mode: gameMode,
          player_fingerprint: localStorage.getItem("playerFingerprint"),
        }),
      })
